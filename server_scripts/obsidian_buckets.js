ItemEvents.rightClicked('kubejs:obsidian_bucket', event => {
   ...
   if (!player.creative) {
      if (target.block.id == 'minecraft:lava' && target.block.properties.level == 0) {
      ...
      if (target.block.id == 'minecraft:water' && target.block.properties.level == 0) {
         target.block.set('minecraft:air')
         item.count--
      ...
      if (target.block.id == 'minecraft:powder_snow') {
         target.block.set('minecraft:air')
         item.count--
...
ItemEvents.rightClicked('kubejs:obsidian_water_bucket', event => {
   ...
   if (!player.creative) {
      ...
      if (target.type == 'BLOCK' && target.block.id != 'minecraft:water' && target.block.id != 'minecraft:lava') {
         let placePos = target.block.pos.relative(target.facing)
